...
async def update_trip(trip_id:int,data:TripCreate,admin_id:int,db:AsyncSession=Depends(get_db)):
    admin = await check_admin(db, admin_id)
    if not admin:
        raise HTTPException(status_code=404, detail="Admin not found")
...
async def delete_trip(admin_id:int,trip_id:int,db:AsyncSession=Depends(get_db)):
    admin = await check_admin(db, admin_id)
    if not admin:
        raise HTTPException(status_code=404, detail="Admin not found")
    trip = await TripService.delete_trip(db,trip_id)
    if not trip:
        raise HTTPException(status_code=404, detail="Trip not found")
    return {
